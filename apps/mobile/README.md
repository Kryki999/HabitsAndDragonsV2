...
- Zustand **habits/** + **hero/** + **world/** (hub|map|interior+floor|location|encounter, discovered flags, cleared bosses). Fight loot lands in `hero` inventory. Global account bar (V1 `TabsWithTopBar`: avatar / Lv / gold+keys pills / mail stub / settings) sits above every tab and reads `hero`.
