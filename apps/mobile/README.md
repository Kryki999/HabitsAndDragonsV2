...
- Tab **World** = playground: kingdom map (fog + pins) → Crownhaven hub → location windows (icon dock) → Gutterjack stub, on a thin `world/` store
- Zustand **habits/** + **hero/** + **world/** (hub|map|location, playerLevel, unlock set, gutterjackCleared)
...
V1 raster assets were gitignored (`assets/` in V1). Hero slots use Lucide glyphs. World stills live flat in `apps/mobile/assets/images/` (`world map.png`, location PNGs). Pin coordinates: `world/catalog.ts` + `world/layout.ts`.
