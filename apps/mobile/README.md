...
- Zustand **habits/** + **hero/** + **world/** (hub|map|interior+floor|location|encounter, discovered flags, cleared bosses). Fight loot lands in `hero` inventory. Global account bar (avatar / level / gold / keys) sits above the tabs and reads `hero`.
