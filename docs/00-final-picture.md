...
Cienki scaffold: `apps/mobile` (Expo Router, 5 tabów z `18`). Tab **Questy** = V1 Castle UI na domain store `habits`. Tab **Hero** = V1 Hero UI (EN) na domain store `hero` (hex demo + equipment shell). Tab **World** = playground mapa królestwa → Crownhaven → tawerna (Ground + winda) → Cellar / Gutterjack fight (store `world` + `combat/`: approach still, win%, clash, outcome, CS loot roll). Social / Mentor = later. V1 nie jest kotwicą architektury — [`reference/v1-to-v2-architecture-audit.md`](reference/v1-to-v2-architecture-audit.md).
