...
Pięć tabów: **Questy · World · Hero · Społeczność · Mentor**. Questy = V1 Castle UI na store `habits`. Hero = V1 Hero UI na cienkim store `hero` (EN). World = playground mapa (mgła + piny) → Crownhaven → lokacje (store `world`). Społeczność / Mentor = placeholdery.
