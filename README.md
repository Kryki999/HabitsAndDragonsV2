...
Pięć tabów: **Questy · World · Hero · Społeczność · Mentor**. Questy = V1 Castle UI na store `habits`. Hero = V1 Hero UI na cienkim store `hero` (EN). World = playground Crownhaven → mapa → Gutterjack (store `world`). Społeczność / Mentor = placeholdery.
