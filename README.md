...
Pięć tabów: **Questy · World · Hero · Społeczność · Mentor**. Questy = V1 Castle UI na store `habits`. Hero = V1 Hero UI na cienkim store `hero` (EN). World = playground mapa → Crownhaven → Gutterjack fight (store `world` + `combat/`). Społeczność / Mentor = placeholdery.
